...
module.exports = async ({ usersData }, { userRepositoryMySQL }) => {
  ...
  const studentExist = await userRepositoryMySQL.getUser(idUser);
  if (!studentExist) {
  ...
  // Se comprueba que el usuario con idUser es un estudiante
  const hasRoleAdmin = await userRepositoryMySQL.getRole({ userRoleData: { UserId: idUser, role: ROLES.STUDENT } });
  if (!hasRoleAdmin) {
    throw new ResponseError(TYPES_ERROR.ERROR, 'El usuario no tiene rol de estudiante', 'role_not_assigned');
  }
